...
endButtonElement.addEventListener('click', () => {
  for(let i = 1; i <= NUMBER_OF_PLAYERS; i++) {
    const playerName = getPlayerName(i - 1);
    gameScoreBoard.saveGameScore(playerName, scores[i - 1]);
    scores[i - 1] = 0;
  }
  const topTenScoreBoardElement = document.getElementById('top-10-score-board');
  topWordAndGameScoreBoard.render(topTenScoreBoardElement);
});
